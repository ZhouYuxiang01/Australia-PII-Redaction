...
def test_safe_postprocess_keeps_gender_and_pronoun_labels_for_hybrid_policy() -> None:
    text = "gender: male\nsex: female\npronouns: she/her"
    spans = [
        Span(
    ...
    assert [(span.type, span.value) for span in cleaned] == [
        ("GENDER", "male"),
        ("GENDER", "female"),
        ("PRONOUN", "she/her"),
    ]
...
def test_hybrid_policy_declares_all_postprocess_flags() -> None:
    policy = json.loads((PROJECT_ROOT / "configs" / "policies" / "hybrid-80class-v2-4b.json").read_text())
    assert policy["postprocess"] == {
        ...
        "add_url_encoded_emails": True,
            "add_contextual_identifier_spans": True,
...
def test_vehicle_context_conflicts_follow_hybrid_policy() -> None:
    ...
    assert [(s.type, s.decision) for s in decided] == [("VEHICLE_ID", "AUTO_REDACT")]

    rescued, _ = safe_postprocess_spans(text, [], {"postprocess": {}})
    decided = apply_policy(rescued, policy)
    assert [(s.type, s.value, s.decision) for s in decided] == [("VEHICLE_ID", "VIC987XYZ", "AUTO_REDACT")]
