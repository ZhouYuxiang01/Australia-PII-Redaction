...
def _default_pii_project_root() -> Path:
    ...
    for candidate in (
        here.parents[3] / "hybrid-pii-model-runtime",
        here.parents[2] / "hybrid-pii-model-runtime",
    ):
        if candidate.is_dir():
            return candidate
    raise FileNotFoundError(
        "Could not locate hybrid-pii-model-runtime. Set REDACTION_PII_PROJECT_ROOT "
